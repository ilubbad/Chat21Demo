...
public class ContactsSynchronizer {
    ...
    public ContactsSynchronizer(String firebaseUrl, String appId) {
        ...
        Log.d(DebugConstants.DEBUG_CONTACTS_SYNC, "contactsNode : " + contactsNode.toString());
    }
                            ...
                            new Thread(new Runnable() {
                                public void run() {
                                    ...
                                    try {
                                        ...
                                        Log.d(DebugConstants.DEBUG_CONTACTS_SYNC, "ContactsSynchronizer.connect.onChildAdded.contact : " + contact);
                                        addContact(contact);
                                    } catch (ChatFieldNotFoundException cfnfe) {
                                        Log.w(DebugConstants.DEBUG_CONTACTS_SYNC, "Error decoding contact on onChildAdded " + cfnfe.getMessage());
...
//                            Log.v(DEBUG_CONTACTS_SYNC, "ContactsSynchronizer.connect.onChildChanged");
                    ...
                    try {
                        ...
                        Log.d(DebugConstants.DEBUG_CONTACTS_SYNC, "ContactsSynchronizer.connect.onChildChanged.contact : " + contact);
                        updateContact(contact);
                    } catch (ChatFieldNotFoundException cfnfe) {
                        Log.w(DebugConstants.DEBUG_CONTACTS_SYNC, "Error decoding contact on onChildChanged " + cfnfe.getMessage());
...
//                    }.execute(dataSnapshot);
                ...
                public void onChildRemoved(DataSnapshot dataSnapshot) {
                    ...
                    try {
                        ...
                        Log.d(DebugConstants.DEBUG_CONTACTS_SYNC, "ContactsSynchronizer.connect.onChildRemoved.contact : " + contact);
                        ...
                        Log.w(DebugConstants.DEBUG_CONTACTS_SYNC, "Error decoding contact on onContactRemoved " + cfnfe.getMessage());
                    } catch (Exception e) {
                        if (contactListeners != null) {
                            for (ContactListener contactListener : contactListeners) {
...
//                Log.d(TAG, "observeMessages.onCancelled");
    ...
    private void saveOrUpdateContactInMemory(IChatUser contact) {
        Log.d(DebugConstants.DEBUG_CONTACTS_SYNC, "saveOrUpdateContactInMemory  for contact : " + contact);

        int index = contacts.indexOf(contact);

        if (index > -1) {
            contacts.set(index, contact);
            Log.v(DebugConstants.DEBUG_CONTACTS_SYNC, "contact " + contact +
                    "updated into contacts at position " + index);

        } else {
            contacts.add(contact);
            Log.v(DebugConstants.DEBUG_CONTACTS_SYNC, "contact " + contact + "is not found into contacts." +
...
//        }
    ...
    public static IChatUser decodeContactSnapShop(DataSnapshot dataSnapshot) throws ChatFieldNotFoundException {
        ...
        Log.v(DebugConstants.DEBUG_CONTACTS_SYNC, "decodeContactSnapShop.contact : " + contact);
